...
class Game {
    ...
    updateBoard() {
        this.squares.forEach((square, index)=>{
            ...
            if (value) {
                const className = value > 2048 ? 'field-cell--super' : `field-cell--${value}`;
    ...
    getState() {
        return this.boardState.map((row)=>[
                ...row
            ]);
    }
    getScore() {
    ...
    generate() {
        ...
        for(let outerRowIdx = 0; outerRowIdx < 4; outerRowIdx++){
            for(let innerColIdx = 0; innerColIdx < 4; innerColIdx++)if (this.boardState[outerRowIdx][innerColIdx] === 0) emptySquares.push({
                rowIdx: outerRowIdx,
                colIdx: innerColIdx
            });
        }
    ...
    addTouchEvents() {
        ...
        this.gameBoard.addEventListener('touchend', (e)=>{
            ...
            } else if (deltaY > 0) this.moveDown();
            else this.moveUp();
            this.generate();
            this.result();
        });
    ...
    getStatus() {
        if (this.boardState.some((row)=>row.includes(2048))) this.status = 'won';
        else if (this.boardState.some((row)=>row.includes(0))) this.status = 'playing';
        else {
            for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++){
                const cell = this.boardState[i][j];
                if (i > 0 && cell === this.boardState[i - 1][j] || i < 3 && cell === this.boardState[i + 1][j] || j > 0 && cell === this.boardState[i][j - 1] || j < 3 && cell === this.boardState[i][j + 1]) {
                    this.status = 'playing';
                    return 'playing';
                }
            }
    ...
    result() {
        this.messageLose.classList.add('hidden');
        this.messageWin.classList.add('hidden');
        if (this.status === 'lost') this.messageLose.classList.remove('hidden');
        else if (this.status === 'won') this.messageWin.classList.remove('hidden');
    }
    move(direction) {
        if (!this.gameStarted) return;
        ...
        for(let i = 0; i < 4; i++){
            const line = isHorizontal ? this.boardState[i].slice() : this.boardState.map((row)=>row[i]);
            if (isReverse) line.reverse();
            const filtered = line.filter((val)=>val !== 0);
            const merged = [];
            for(let j = 0; j < filtered.length; j++)if (filtered[j] === filtered[j + 1]) {
            ...
            if (isHorizontal) {
                ...
                for(let j = 0; j < 4; j++)if (this.boardState[j][i] !== merged[j]) {
